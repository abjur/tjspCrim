...
Tabela de frequências das classes processuais.
...
d_cjsg %>%
  mutate(class = ifelse(class %in% tabela$class, class, "Outros")) %>% 
  count(class)
...
Nessa pesquisa, consideramos no escopo somente apelações realizadas contra o Ministério Público. Assim, incluímos na base somente processos em que o réu apelava para pedir cancelamento, redução de pena, etc. Após
esse filtro ficamos com uma base contendo 57.625 processos.

Para obter as decisões dos processos, fizemos um *text mining* dos textos das decisoes, extraindo os resultados a partir de regras lógicas e expressões regulares. Os resultados não são totalmente a prova de
erros, mas é provavel que as classificações estejam próximas da realidade.
...
A Tabela **XXX - ALTERAR** mostra o a distribuição das decisões dos processos. É interessante notar que a taxa de decisões desfavoráveis é de aproximadamente 50\%, o que vai de encontro com o resultado apresentado
por Priest \& Klein (1984).

Tabela de frequências dos resultados dos processos.

```{r}
# dados Júlio
d_final$decisao <- factor(d_final$decisao, levels = c('provido', 'parcialmente', 'negaram', 'outros'))
d_final %>%
  count(decisao)

# dados completos
d_cposg_cjsg$decision <- factor(d_cposg_cjsg$decision, levels = c('Provido', 'Parcialmente', 'Negaram', 'Outros'))
...
Tabela de frequências dos resultados dos processos segundo tipo de órgão julgador.
```{r}
# Dados Júlio
d_final %>% 
...
Tabela de frequências dos resultados dos processos segundo órgao julgador.
```{r}
# Dados Júlio
d_final %>% 
...
Tabela de frequências dos assuntos dos processos segundo órgao julgador.
```{r}
# Dados Júlio
d_final %>% 
...
A Tabela **XXX** mostra os resultados para a base de dados homogeneizada. Podemos observar que, mesmo realizando esse ajuste, os resultados das câmaras permanecem distintos.

(Tabela de frequências dos resultados dos processos segundo órgao julgador, a partir de base homogeneizada.)
```{r}
...
Tabela de frequências dos resultados dos processos segundo relator.
```{r}
# Dados Júlio
d_final %>% 
...
A Figura **XXX** mostra as proporções de resultados dos processos em cada mês. É possível notar que entre os meses de junho e setembro houve um aumento significativo na proporção de recursos negados, caindo novamente
